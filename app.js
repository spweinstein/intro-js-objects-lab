...
*/
game.party.push(pokemon[141], pokemon[149], pokemon[144]);
// console.log(game.party);
...
game.catchPokemon(pokemon.find((pokemonObj) => pokemonObj.name == "Machamp"));
...
game.catchPokemon = (pokemonObj) => {
  game.party.push(pokemonObj);
  game.items.find((item) => item.name === "pokeball").quantity--;
  //If more than 6 pokemon in party, add first pokemon to collection
  if (game.party.length > 6) {
    game.collection.push(game.party.shift());
  }
};
game.catchPokemon(pokemon.find((pokemonObj) => pokemonObj.name === "Moltres"));
...
game.catchPokemon = (pokemonName) => {
  ...
  let pokemonObj = pokemon.find(
    (pokemonObj) => pokemonObj.name.toLowerCase() === pokemonName.toLowerCase()
  );
  if (!pokemonObj) {
...
game.catchPokemon("sakldjasljda");
console.log(game.party);
...
pokemon.forEach((pokemonObj) => {
  if (!pokemonTypes.includes(pokemonObj.type)) {
    pokemonTypes.push(pokemonObj.type);
    pokemonsByType[pokemonObj.type] = [];
  }
  pokemonsByType[pokemonObj.type].push(pokemonObj);
});
